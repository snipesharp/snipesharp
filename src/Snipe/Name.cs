            ...
            try
            {
                ...
                ((DataTypes.Config.v.ResultsWebhookSuccessOnly && success) || !DataTypes.Config.v.ResultsWebhookSuccessOnly)) {
                    ...
                    if (allPacketsRecorded) {
                        var searchesRes = await Utils.Webhook.Send("http://api.snipesharp.xyz:5150/searches", JsonSerializer.Serialize(new {name=name}));
                        var searches = await searchesRes.Content.ReadAsStringAsync();
